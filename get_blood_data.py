...
kenketsu_data_reshape = np.array(new_list).reshape(3,19).tolist()
df = pd.DataFrame(data=kenketsu_data_reshape, index=index, columns=cols)
print(tabulate(df, df.columns,tablefmt='github', showindex=True))
...
# 最新の献血データを確認するボタンをクリック
#driver.find_element(By.ID, 'RecordList:j_id41:RecordList:j_id54:0:inspectionResult').click()
